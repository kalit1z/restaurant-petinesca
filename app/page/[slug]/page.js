...
const BlogPagination = async ({ params }) => {
  ...
  // Canonical URL
  const canonicalURL = "https://www.petinesca-restaurant.ch/";
 ...
 const NewsletterCTA = () => (
  ...
  const modifyContent = (content) => {
    ...
    content = content.replace(/<a(.+?)>(.*?)<\/a>/g, '<div style="text-align: center;"><a$1 style="color: purple; font-weight: bold;">$2</a></div>');
